    ...
    <>
      <div className="p-4  xl:ml-80  h-[calc(96vh-32px)]  max-w-screen rounded-xl transition-transform duration-300 xl:translate-x-0 " >
        ...
        <div className='text-sm  text-black flex flex-col justify-center w-full bg-white p-6 shadow-lg rounded  mt-3'>
          ...
          {!isLoading ? (
            <div className="mt-5 w-full rounded-lg shadow-lg h-[calc(100vh-200px)] border border-gray-200 bg-gray-100">
                ...
                <Accordion hideIndicator >
                  {locaciones.map((ubicacion) => (
                    <AccordionItem
                      ...
                      className='w-full rounded-lg font-sans p-1 hover:shadow-md transition-all duration-200 mb-3  text-base bg-white border border-zinc-200 shadow-sm'
                    >
                      <div className="grid grid-cols-[repeat(auto-fill,minmax(80px,1fr))] gap-1 mb-3 px-11">
                        ...
                        {camerasPorLocacion[ubicacion.locacion]?.map((camera) => (
                          <ReactLink to={`/registros/camara/${camera}`} key={camera}>
                            ...
                            >
                              <p className='text-lg font-sans'>{camera}</p>
                            </Button>
                          </ReactLink>
                        ))}
                      </div>
